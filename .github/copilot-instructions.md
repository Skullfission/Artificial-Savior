...
- **Cheat codes** (`CHEAT_CODES`, currently `EDGE` and `MICO`) have two entry paths: (1) silent rolling 8-char `state.cheatBuffer` on the title screen for keyboard users, and (2) a 4-character pause-menu input (`state.cheatEntry`, geometry via `cheatBoxRects()`, submitted by `submitCheatEntry()`) for touch users. The pause-menu has no on-screen ENTER button — touch users submit by tapping the FIRE button while paused with a cheat entry open. Both paths call `applyCheat(state.player)` and set `state.cheatBanner = 2.5`. `applyCheat()` also re-runs inside `reset()` so Retry preserves them. Don't add cheat hints to on-screen UI; pause-menu misses are silent by design (boxes are NOT cleared on miss).
...
`#touch` and its children (`#tc-stick`, `#tc-knob`, `#tc-fire`, `#tc-weapons`, `#tc-action`) are hidden by default and only become visible via the `@media (pointer:coarse)` rule in `style.css`. The single multi-purpose `#tc-action` button toggles between Start / Pause (II) / Resume (▶) / Retry / OK based on `state` flags. If you add new on-screen controls, add the corresponding `pointer:coarse` styling or they will be invisible on phones/tablets. The action button label is driven by `actionLabel()` and changes with `state` flags (paused/gameOver/victory/entry) — update that function rather than writing to the DOM directly. The `#tc-fire` button doubles as the cheat-code submit (ENTER) when `state.paused && state.cheatEntry`; in that case its pointerdown calls `submitCheatEntry()` instead of adding `" "` to `keys`.
